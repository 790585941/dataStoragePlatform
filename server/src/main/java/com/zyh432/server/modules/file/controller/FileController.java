package com.zyh432.server.modules.file.controller;
...
public class FileController {
    ...
    public Data<List<DataStoragePlatformUserFileVO>> list(@NotBlank(message = "父文件夹ID不能为空") @RequestParam(value = "parentId", required = false) String parentId,
        ...
        if (!Objects.equals(FileConstants.ALL_FILE_TYPE,fileTypes)){
            fileTypeArray= Splitter.on(DataStoragePlatformConstants.COMMON_SEPARATOR).splitToList(fileTypes).stream().map(Integer::valueOf).collect(Collectors.toList());
        }
        ...
        List<DataStoragePlatformUserFileVO> result= iUserFileService.getFileList(context);
    ...
    public Data<String> createFolder(@Validated @RequestBody CreateFolderPO createFolderPO){
        ...
        return Data.data(IdUtil.encrypt(fileId));
    }
    ...
    @PutMapping("file")
    public Data updateFilename(@Validated @RequestBody UpdateFilenamePO updateFilenamePO) {
    ...
    @DeleteMapping("file")
    public Data deleteFile(@Validated @RequestBody DeleteFilePO deleteFilePO) {
    ...
    public Data secUpload(@Validated @RequestBody SecUploadFilePO secUploadFilePO) {
        ...
        return Data.fail("文件唯一标识不存在，请手动执行文件上传");
    }
    ...
    public Data upload(@Validated FileUploadPO fileUploadPO) {
    ...
    public Data<FileChunkUploadVO> chunkUpload(@Validated FileChunkUploadPO fileChunkUploadPO) {
    ...
    public Data<UploadedChunksVO> getUploadedChunks(@Validated QueryUploadedChunksPO queryUploadedChunksPO) {
    ...
    public Data mergeFile(@Validated @RequestBody FileChunkMergePO fileChunkMergePO) {
    ...
    public Data<List<FolderTreeNodeVO>> getFolderTree() {
        QueryFolderTreeContext context = new QueryFolderTreeContext();
        context.setUserId(UserIdUtil.get());
        List<FolderTreeNodeVO> result = iUserFileService.
    ...
    public Data transfer(@Validated @RequestBody TransferFilePO transferFilePO) {
    ...
    public Data copy(@Validated @RequestBody CopyFilePO copyFilePO) {
    ...
    public Data<List<FileSearchResultVO>> search(@Validated FileSearchPO fileSearchPO) {
        ...
        if (StringUtils.isNotBlank(fileTypes) && !Objects.equals(FileConstants.ALL_FILE_TYPE, fileTypes)) {
            List<Integer> fileTypeArray = Splitter.on(DataStoragePlatformConstants.COMMON_SEPARATOR).splitToList(fileTypes).stream().map(Integer::valueOf).collect(Collectors.toList());
            context.setFileTypeArray(fileTypeArray);
        }
