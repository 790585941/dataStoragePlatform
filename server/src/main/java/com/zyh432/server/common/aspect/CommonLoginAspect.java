package com.zyh432.server.common.aspect;
...
public class CommonLoginAspect {
     ...
     * 切点表达式
     */
    private final static String POINT_CUT = "execution(* com.zyh432.server.modules.*.controller..*(..))";
    ...
    public Object loginAuthAround(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        if (checkNeedCheckLoginInfo(proceedingJoinPoint)) {
            ...
            if (!checkAndSaveUserId(request)) {
                ...
                return Data.fail(ResponseCode.NEED_LOGIN);
            }
    ...
    private boolean checkAndSaveUserId(HttpServletRequest request) {
        ...
        if(StringUtils.equals(accessToken, redisAccessToken)){
            saveUserId(userId);
            return true;
        }
        return false;
