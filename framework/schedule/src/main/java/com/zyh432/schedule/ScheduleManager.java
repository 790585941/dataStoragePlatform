...
public class ScheduleManager {
    ...
    public String changeTask(String key, String cron) {
        if (StringUtils.isAnyBlank(key, cron)) {
            throw new DataStoragePlatformFrameworkException("定时任务的唯一标识以及新的执行表达式不能为空");
        }
        ScheduleTaskHolder holder = cache.get(key);
        if (Objects.isNull(holder)) {
            throw new DataStoragePlatformFrameworkException(key + "唯一标识不存在");
        }
        stopTask(key);
